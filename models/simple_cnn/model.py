...
class SimpleCNN(torch.nn.Module):
    def __init__(
        ...
        state_vector_size=4,
        state_vector_replication=10,
        fc1_size=4096,
    ):
        super(SimpleCNN, self).__init__()
        self.base_cnn, base_cnn_raster_size = construct_base_cnn()
        self.fc1 = torch.nn.Linear(
            base_cnn_raster_size + (state_vector_size * state_vector_replication),
            fc1_size,
        )
        self.fc2 = torch.nn.Linear(
            fc1_size, num_modes * (2 * predictions_per_mode + 1)
        )  # 2 for x, y and 1 for confidence
    ...
    def forward(self, raster_image, state_vector, latent_output=""):
        ...
        probabilities, predictions = torch.split(
...
def loss_function(
    ...
    probabilities,
    target_prediction,
    prediction_loss_weight=torch.tensor(1.0),
):
    ...
    for batch_i, batch in enumerate(predictions):
        ...
        for mode_i, mode in enumerate(batch):
            distances[mode_i] = _trajectory_distance(
                predictions[batch_i][mode_i], target_prediction[batch_i]
            )
